...
OPS="$(show admin-rbac-ops.yaml)"
grep -q '^kind: Role$' <<<"$OPS" || fail "ops write RBAC must be a namespaced Role"
! grep -q 'kind: ClusterRole' <<<"$OPS" || fail "ops write RBAC must never be cluster-wide"

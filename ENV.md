 Where every environment variable lives
  ...
  There are five files, all on this server at /home/ubuntu/atclub_analytics/, all mode 600 and gitignored:
  ...
  ├─────────────────────────────────┼────────────────────────────────────────────────────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────┤
  │ infra/values.prod.yaml          │ non-secret env under analytics.env: — SIGNUP_ENABLED, COOKIE_SECURE, LOG_LEVEL, ClickHouse URL/user/db │ → ConfigMap myampix-analytics-config                  │
  └─────────────────────────────────┴────────────────────────────────────────────────────────────────────────────────────────────────────────┴───────────────────────────────────────────────────────┘
  ...
  mountPath: /var/run/secrets/google   readOnly: true   defaultMode: 0400

  The JSON never enters the repo, the image, or the values file — only the Secret, projected read-only and readable solely by the container's user. With enabled: false (today) the chart renders zero references to it, so nothing changes until you flip it.

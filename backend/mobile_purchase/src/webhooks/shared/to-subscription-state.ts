 ...
 * docstring for why those are excluded). Store-agnostic: M2b (Apple) and M3b (Google) both own
 * resolving *which* row an event applies to and merging the reducer's output back with those
 * excluded columns — this is the read-side half of that job, shared by both (originally M2b-only,
 * relocated here by M3b per the design brief's "extract the store-agnostic core" preference —
 * `apple/to-subscription-state.ts` re-exports this so existing Apple call sites/tests don't move).
 *
 * `lastEventAt` is nullable in the schema (a defensive DB-level allowance) but always populated by
 * the write path (`createInitialState` in the reducer always sets it on the first persisted row).
 * Falls back to the epoch when somehow null so the reducer's ordering guard never spuriously blocks
 * an otherwise-valid event against such a row, rather than mis-comparing against `undefined`.
 */

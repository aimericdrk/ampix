...
describe('ErasureKeyGuard', () => {
  ...
  it('throws a 401 problem when the header is missing, wrong, or not a string (authorization)', () => {
    const guard = new ErasureKeyGuard({ erasureApiKey: KEY } as AppConfig);
    for (const headers of [{}, { 'x-erasure-key': 'wrong' }, { 'x-erasure-key': `${KEY}x` }, { 'x-erasure-key': [KEY] }]) {

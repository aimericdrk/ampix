...
describe('OfferingsService', () => {
  ...
  it('setCurrent 404s for a cross-project or non-existent offering', async () => {
    ...
    await expect(service.setCurrent(otherProjectId, offering.id)).rejects.toMatchObject({ problem: { status: 404 } });
  ...
  it('addPackage 404s for a non-existent/cross-project offering', async () => {
    const otherProjectId = randomUUID();
    const offering = await service.create(projectId, { identifier: 'foreign-offering', displayName: 'Foreign' });

    await expect(
      service.addPackage(otherProjectId, offering.id, {
        identifier: '$rc_monthly',
        packageType: 'MONTHLY',
        productId,
      }),
  ...
  it('addPackage rejects a productId that belongs to another project (400)', async () => {
    ...
    const otherApp = await prisma.app.create({
      data: {
        ...
        name: 'Android',
        platform: 'ANDROID',
        packageName: `com.other.app.${randomUUID()}`,
        publicSdkKey: `mp_pub_other_${randomUUID()}`,
      },
    });
    ...
    const offering = await service.create(projectId, {
      identifier: 'foreign-product-offering',
  ...
  it('remove() deletes an offering and 404s for a cross-project or non-existent offering', async () => {
    ...
    await expect(service.remove(otherProjectId, offering.id)).rejects.toMatchObject({ problem: { status: 404 } });

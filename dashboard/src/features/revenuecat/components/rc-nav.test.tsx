...
describe('MyRevenueCat nav', () => {
  ...
  it('shows the RC clone pages even when RC is not connected — the self-hosted clone has no connect gate', async () => {
  ...
  it('carries no Integration settings item — RC settings is a scope of Project settings', async () => {
    authStore.setSession(VALID_ACCESS_TOKEN, TEST_USER);
    renderApp(`/projects/${TEST_PROJECT.id}/rc/overview`);
    const nav = within(await screen.findByRole('navigation', { name: 'Primary' }));
    await nav.findByText('Overview');
    expect(nav.queryByText('Integration settings')).not.toBeInTheDocument();
    // The way in is the global "Project settings" link, which sits outside the tool's section nav.

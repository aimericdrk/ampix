...
import { revokeSession, sessionCookie } from '@/lib/session';
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  if (s) {
    await revokeSession(prisma, s.session.id);
    const { ip } = await requestMeta();
    await writeAudit(prisma, 'logout', s.user.id, {}, ip);
  }
  // Always clear the cookie and land on /login, session or not.

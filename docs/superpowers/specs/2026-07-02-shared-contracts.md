...
- Postgres table `screen_captures(id uuid, project_id fk, screen_name, storage_path text, content_type, width int, height int, app_version, image_hash, captured_at, updated_at)` — **no bytea**; `storage_path` is the object path in the bucket: `{organization_id}/{project_id}/screen/{screen_name}/{app_version}.jpg` — fully tenant-isolated (org + project ids are UUIDs; `screen_name`/`app_version` are URI-encoded so they can't escape their folder). The service resolves the project's `org_id` (a `project.findUnique`) at store time. Unique **`(project_id, screen_name, app_version)`** — exactly ONE image per screen per app version. `POST /ingest/screenshots` UPSERTs: put the bytes to Firebase at the deterministic path (overwrites), upsert the metadata row. Storage bounded (#screens × #versions). Prisma migration `v2_screen_captures`.
...
- Read (membership-gated, viewer+): `GET /api/v1/projects/:projectId/screens` → `{screens:[{screen_name, capture_count, latest_captured_at, width, height, latest_image_hash, latest_app_version}]}` (`latest_image_hash`/`latest_app_version` come from the newest capture per screen; the dashboard content-addresses the image URL with the hash so a retake busts the client cache). `GET /api/v1/projects/:projectId/screens/:screenName/image?app_version=<optional>&hash=<optional>` → **the JPEG image** (Content-Type image/jpeg, Cache-Control), newest version if unspecified — the endpoint either streams the bytes proxied from Firebase Storage OR `302`-redirects to a short-lived signed URL (implementation choice; the RESPONSE the client/browser sees is the image). `404` if none.
...
Body: `{ "screen_name": "checkout", "date_range": {from,to}, "grid": { "cols": 20, "rows": 40 }, "filters": [], "distinct_ids"?: ["u_42","<anon>"] }` (cols/rows 1..100). Engine: over `$tap` events with `$screen_name = {screen:String}`, normalize each tap to `[0,1]²` via `$pos_x / screen_width`, `$pos_y / screen_height` (context columns; skip rows with 0 width/height), bucket into the `cols×rows` grid, `count()` per cell. → `{ "screen_name", "total": N, "cells": [ { "cx", "cy", "count" } ] }` (cx∈0..cols-1, cy∈0..rows-1; empty cells omitted). Powers the heatmap overlay on the screen's screenshot. Optional `distinct_ids` (≤1000) adds a `distinct_id IN {…:Array(String)}` filter on the RAW column (bound param, not canonicalized) for an identity-correct per-user heatmap — the caller passes a user's §17 identity set (canonical id + aliased anon_ids, e.g. from `GET /users/:distinctId`) so taps under both the anon and identified ids are counted.

### POST /query/screen-paths
Like §15 flows BUT nodes are **screens** (the `$screen_name` property of `$screen_view` events), not event names — the existing flows engine keys nodes on event name, which can't distinguish screens. Body: `{ "anchor_screen"?: "home", "direction": "forward"|"backward", "date_range", "steps": 1..5, "max_nodes_per_step": 1..20, "unit": "session"|"user", "distinct_ids"?: ["<canonical id + aliased anon_ids>"] }` (omit `anchor_screen` → start from the top entry screens). Per unit, order `$screen_view` by timestamp, take the `$screen_name` sequence, aggregate transitions `uniqExact(uid)`, top-N per step + `$other`/`$end`. → same Sankey shape as §15 flows (`{nodes:[{id,step,event:screen_name,value}], links:[{source,target,value}]}`). This is the user-path-map data source. Optional `distinct_ids`: the same §17 identity-set filter as the click-heatmap (`e.distinct_id IN (…)`, bound `Array(String)`) — pass the canonical id + its aliased anon_ids (from `GET /users/:distinctId`) to render an identity-correct per-user path map.

### POST /query/histogram (added 2026-07-08, feat-09 §3.1/T1)
Body: `{ "event": "$session_end", "property": "$duration_ms", "bins"?: 20 (2..50, default 20), "date_range": {from,to}, "filters"?: [] }` (≤20 filters, same §14 shape). `property` resolves through the §14 `resolveProperty` whitelist/bound-custom-key rule (never interpolated); the resolved expression is cast `toFloat64OrNull(...)` before bucketing. Engine: over `events` rows with the bound `event`/date-range/filters, drop null/non-finite values, then `histogram({bins})(value)` (ClickHouse's adaptive histogram — `bins` is OUR schema-validated int, embedded as a literal since the aggregate function requires it as one, never caller text) plus `count()`, `min()`, `max()`, `avg()`, `quantile(0.5)`, `quantile(0.9)` of the same value. → `{ "buckets": [ {"lower","upper","count"} ] (height rounded to an integer count), "total", "min", "max", "mean", "p50", "p90" }`. No matching/finite-valued events → `{ "buckets": [], "total": 0, "min": 0, "max": 0, "mean": 0, "p50": 0, "p90": 0 }`. Powers the Distributions page's histogram chart (session-length/purchase-value presets + a custom event+property mode).
...
### GET /metrics/revenue?from=&to=
Mirrors the §14 sessions-summary pattern, off `$in_app_purchase` events' `$price`/`$product_id` properties. Engine: `total_revenue = sum(JSONExtractFloat(toJSONString(properties), '$price'))`, `purchases = count(DISTINCT insert_id)`, `paying_users = uniqExact(<canonical uid>)` (§17 `canonicalization()`); `arppu = total_revenue / paying_users` and `avg_purchase_value = total_revenue / purchases` (both `0` when their denominator is `0`, sidestepping ClickHouse's NaN-on-empty-avg). → `{ "total_revenue", "purchases", "paying_users", "arppu", "avg_purchase_value", "by_day": [ {"t","revenue","purchases"} ] (zero-filled onto the date grid), "by_product": [ {"product_id","revenue","purchases"} ] (top 10 by revenue via `JSONExtractString(...,'$product_id')`, empty product ids excluded) }`.
